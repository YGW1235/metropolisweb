...
| 중간 | 공개 raw read 정책 순서 의존 | `migrations/004_posts_comments.sql`은 `public can read visible debate posts/comments` 정책을 생성합니다. `patches/003_remove_public_raw_debate_reads.sql`과 `017_enforce_private_debate_raw_reads.sql`이 이를 제거해 anonymous RPC 중심 구조를 보장합니다. | 신규 DB 구성 시 anonymous RPC와 raw read 제거 patch를 번호 순서대로 적용합니다. 운영 DB에서는 정책 목록에서 raw table 공개 read가 남아 있지 않은지 확인합니다. |
...
- `supabase/patches/017_enforce_private_debate_raw_reads.sql` 적용 필요 여부
- anonymous RPC grant signature가 실제 DB 함수 signature와 일치하는지 여부
