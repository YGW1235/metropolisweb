...
이 문서는 `supabase/migrations`, `supabase/patches`, `supabase/README.md`에 있는 repo 파일만 기준으로 점검한 결과입니다. 실제 운영 Supabase DB에는 이미 수동 보정되었거나 더 최신 정의가 적용되어 있을 수 있습니다. 이번 점검에서는 DB에 접속하지 않았고, SQL을 실행하지 않았으며, SQL 파일도 수정하지 않았습니다.

## 요약

- `join_topic`은 구버전 정의가 repo에 남아 있지만, 최신으로 보이는 `patches/008_fix_join_topic_side_index.sql`은 `p_join_side`, advisory lock, unique 충돌 재시도 로직을 포함합니다.
- `migrations/005_reports_moderation.sql`, `migrations/007_olive.sql` 안에 Markdown 코드펜스가 남아 있어, repo SQL을 새 DB에 그대로 적용하면 구문 오류가 날 위험이 큽니다.
...
| 위험도 | 항목 | 발견 내용 | 추천 조치 |
| --- | --- | --- | --- |
| 높음 | SQL 구문 오류 가능성 | `supabase/migrations/005_reports_moderation.sql`에 Markdown 코드펜스가 여러 곳 남아 있습니다. 이 파일은 `create_report`, `moderate_report`를 포함하므로 신규 DB 부트스트랩이 중단될 수 있습니다. | 다음 SQL 정리 단계에서 코드펜스를 제거하고, SQL Editor 또는 로컬 parser로 구문 확인 후 별도 patch 또는 정리 migration으로 반영합니다. |
| 높음 | SQL 구문 오류 가능성 | `supabase/migrations/007_olive.sql`에도 Markdown 코드펜스가 남아 있습니다. | `water_olive()` 함수 내부의 코드펜스를 제거하고, 신규 DB 재현 절차 전에 검증합니다. |
| 높음 | `join_topic` 구버전 정의 잔존 | `migrations/003_participants.sql`과 `patches/005_user_suspension.sql`에 `join_topic(p_topic_id uuid)` 구버전이 남아 있습니다. 두 정의 모두 `count(*) + 1`로 `side_index`를 계산합니다. | 운영 DB에서 현재 `join_topic` signature가 `join_topic(uuid, text)`인지, `join_topic(uuid)`가 제거되었는지 Supabase Dashboard에서 확인합니다. fresh setup 문서에는 `patches/008_fix_join_topic_side_index.sql` 적용을 필수로 표시합니다. |
...
- `supabase/migrations/003_participants.sql`
  - `join_topic(p_topic_id uuid)` 정의가 있습니다.
  - `topic_participants_topic_side_index_unique` 제약은 `(topic_id, assigned_side, side_index)` unique입니다.
  - `side_index`는 `count(*) + 1`로 계산됩니다.
- `supabase/patches/005_user_suspension.sql`
  - 정지 유저 참가 방지를 추가하면서 `join_topic(p_topic_id uuid)`를 다시 정의합니다.
  - 이 버전도 `count(*) + 1`로 `side_index`를 계산합니다.
...
최신 patch까지 적용된 DB라면 unique 제약과 충돌할 가능성이 낮아졌습니다. 하지만 구버전이 repo에 남아 있으므로, patch 008이 누락된 신규 DB나 운영 DB에서는 동시 참가 시 `topic_participants_topic_side_index_unique` 충돌이 재현될 수 있습니다.
...
| `create_report` | `migrations/005_reports_moderation.sql` | 동일 | 파일 내 코드펜스 때문에 SQL 구문 확인 필요 |
...
1. `supabase/migrations/005_reports_moderation.sql`
   - Markdown 코드펜스 제거
   - `create_report`, `moderate_report` 구문 검증
2. `supabase/migrations/007_olive.sql`
   - Markdown 코드펜스 제거
   - `water_olive()` 구문 검증
3. `supabase/migrations/003_participants.sql`
   - 구버전 `join_topic(p_topic_id uuid)`가 역사적 정의임을 명확히 하거나 consolidated baseline에서 최신 정의로 정리
4. `supabase/patches/005_user_suspension.sql`
   - 구버전 `join_topic(p_topic_id uuid)` 재정의가 patch 008보다 앞선 역사적 단계임을 문서화
5. `supabase/patches/008_fix_join_topic_side_index.sql`
   - 최신 canonical `join_topic`로 표시
6. `supabase/migrations/004_posts_comments.sql`와 `supabase/patches/003_remove_public_raw_debate_reads.sql`
   - raw public read 정책 생성/제거 흐름을 README에 더 명확히 기록
7. `supabase/patches/007_admin_activity_logs.sql`, `010_fix_admin_topic_stats_ambiguous_columns.sql`, `015_fix_admin_security_status_return_type.sql`
   - 최신 canonical 함수 정의 파일로 문서화

## 결론

repo 기준으로는 최신 운영 기능을 위한 patch들이 대체로 존재합니다. 가장 큰 문제는 일부 migration 파일의 SQL 구문 오류 가능성과, `join_topic`/RLS/관리자 함수가 patch 순서에 강하게 의존한다는 점입니다. 실제 운영 DB가 이미 정상일 수 있으므로, SQL을 고치거나 재적용하기 전에는 Supabase Dashboard에서 현재 함수와 정책 상태를 먼저 확인해야 합니다.
